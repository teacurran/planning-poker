...
class RoomRepositoryTest {
    ...
    void testRelationshipNavigationToOwner(UniAsserter asserter) {
        ...
        asserter.assertThat(() -> Panache.withTransaction(() -> roomRepository.findById("room03").flatMap(found ->
            // Fetch the owner separately to verify the relationship
            userRepository.findById(testOwner.userId).map(owner -> {
                assertThat(found).isNotNull();
                assertThat(found.owner).isNotNull();
                assertThat(found.owner.userId).isEqualTo(owner.userId);
    ...
    void testFindInactiveSince(UniAsserter asserter) {
        ...
        // Use HQL mutation query to update lastActiveAt timestamp (bypassing @UpdateTimestamp)
        asserter.execute(() -> Panache.withTransaction(() ->
            io.quarkus.hibernate.reactive.panache.Panache.getSession().flatMap(session ->
                session.createMutationQuery("UPDATE Room SET lastActiveAt = :timestamp WHERE roomId = :roomId")
                    .setParameter("timestamp", twoDaysAgo)
                    .setParameter("roomId", "room16")
                    .executeUpdate()
            )
        ));

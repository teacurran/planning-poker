...
/**
 * Handles round.reveal.v1 messages (host-only).
 * <p>
 * Processes round reveal requests from the host, validates authorization,
 * and delegates to VotingService.
 * </p>
 */
@ApplicationScoped
public class RoundRevealHandler implements MessageHandler {
                ...
                .onItem().transformToUni(isHost -> {
                    if (!isHost) {
                        sendError(session, requestId, 4003, "FORBIDDEN",
                                "Only HOST role can reveal rounds");
                    ...
                    return roundRepository.findLatestByRoomId(roomId)
                            .onItem().transformToUni(round -> {
                                ...
                                // Reveal round via VotingService
                                return votingService.revealRound(roomId, round.roundId)
                                        .onItem().transformToUni(revealedRound -> {
                                            Log.infof("Round revealed successfully: roundId=%s, roomId=%s",
                                        ...
                                        .onFailure().recoverWithUni(e -> {
                                            Log.errorf(e, "Failed to reveal round: %s", e.getMessage());

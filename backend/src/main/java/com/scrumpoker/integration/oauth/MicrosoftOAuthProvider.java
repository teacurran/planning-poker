 ...
 * Handles authorization code exchange and ID token validation for Microsoft Identity Platform.
 * Uses Microsoft's OAuth2 endpoints: login.microsoftonline.com/common
 *
 * Microsoft Identity Platform supports both personal Microsoft accounts and
 * Azure AD organizational accounts via the /common tenant endpoint.
 */
@ApplicationScoped
public class MicrosoftOAuthProvider {

    private static final Logger LOG = Logger.getLogger(MicrosoftOAuthProvider.class);
     ...
     *
     * @param authorizationCode Authorization code from Microsoft OAuth callback
    ...
    public OAuthUserInfo exchangeCodeForToken(String authorizationCode,
        ...
        try {
            ...
            if (response.statusCode() != 200) {
                LOG.errorf("Microsoft token exchange failed with status %d: %s",
            ...
            LOG.error("Network error during Microsoft OAuth token exchange", e);
            throw new OAuth2AuthenticationException(
     ...
     * Performs signature verification using Microsoft's JWKS endpoint and validates expiration.
     *
    ...
    public OAuthUserInfo validateAndExtractClaims(String idToken) {
        ...
        try {
            ...
            // Validate issuer (Microsoft supports multiple issuer formats)
            ...
            if (email == null || email.isEmpty()) {
                throw new OAuth2AuthenticationException(
                        "Missing 'email' or 'preferred_username' claim in ID token",
                        PROVIDER_NAME);
            }
            ...
            LOG.infof("Successfully validated Microsoft OAuth token for user: %s", email);

            return new OAuthUserInfo(subject, email, name, picture, PROVIDER_NAME);

        } catch (ParseException e) {
            LOG.error("Failed to parse or validate Microsoft ID token", e);
            throw new OAuth2AuthenticationException(
     ...
     */
    private boolean isValidMicrosoftIssuer(String issuer) {
